...
**手（指先）**: DWPose の全身 134 点のうち両手 21 点 × 2（手首根元 + 5 指 × 4 関節）も使います。
片手につき指の骨 20 本のうち両画像で共通して取れたものが 6 本以上あれば、その手の指の向きの一致（cos）を計算し、
左右の平均を重み 0.5 で体（重み 1.0）に合成します。ハート型の手と組んだ手のように指の形が違うペアはここで差が付きます。
指の信頼度閾値は 2.5（performance モードの実測: 見えている手 2.3〜5.8、隠れた手 0.5〜2.0）。
rtmlib の専用 Hand モデル（実写の手で学習）はアニメ絵で信頼度 0.5 未満と全滅だったため使っていません。
...
- rtmlib の `performance` モード（RTMW-DW x-l, 入力 384×288, 検出器 yolox_x）を使用。`balanced`（256×192）より関節位置が安定します。
  このモードの ONNX は信頼度が未正規化（可視の目・鼻で 5〜7、隠れた手首・膝で 1.6〜2.8）なので、体の閾値は実測に基づき 2.8 にしています。
- 左右反転 TTA + 複数スケール切り出し: 人物ボックスをスケール 1.0 / 1.25 で切り出し、それぞれ元画像と反転画像で推定し
  （計 4 回）、左右ラベルを戻してから座標を信頼度で加重平均。較正セット 26 枚での実測では、反転のみと比べて
  「同じ絵の増強 vs 別の絵」の AUC が 0.976 → 0.981、両画像で取れる体の関節ペアが平均 8.6 → 10.4 本に増えました
  （3 スケールは微差で処理時間増、短辺 768px への事前拡大はむしろ悪化したため不採用）。
...
| DWPose | limb cos | −0.074 ※ | 0.734 | 0.978 | 0.999 | 0.997 |

※ ポーズは無関係画像がほぼ対象外になるため、0 点アンカーは other_cut の 5 パーセンタイルです。
別カット同士でも limb cos の中央値が 0.73 と高い（アニメのフレームは正面バストアップが多く、首→肩・鼻→目などの向きが揃う）ため、
ポーズの目盛りは「ほぼ一致しないと高得点にならない」厳しめになっています（指を加える前は 0.81 で、指の分だけ別カットの識別が改善）。
左右反転は Depth では別構図（中央値 0.689）、ポーズでも四肢の向きが反転するため下がります（中央値 0.878）。
