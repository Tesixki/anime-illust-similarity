...
ポーズは信頼度 0.5 以上の関節のみ使い、両画像で共通して取れた関節ペアが 4 本未満なら「対象外」として総合から外します。
...
| DWPose | limb cos | −0.016 ※ | 0.852 | 0.984 | 1.000 | 1.000 |

※ ポーズは無関係画像がほぼ対象外になるため、0 点アンカーは other_cut の 5 パーセンタイルです。
別カット同士でも limb cos の中央値が 0.85 と高い（アニメのフレームは正面バストアップが多く、首→肩・鼻→目などの向きが揃う）ため、
...
| `POSE_MODE` | `balanced` | rtmlib のモード（`lightweight` / `balanced` / `performance`） |
